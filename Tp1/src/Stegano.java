...
public class Stegano {
	...
	public static BufferedImage writeTextInImage(String text, int width,
		...
		for (j = pol; j < height && k < text.split(" ").length - 1; j += 40) {
			...
			for (i = 0; i < width && k < text.split(" ").length - 1; i += 40 * l) {
			...
			if(w!=w1&&h!=h1&&t!=t1&&w!=w2&&h!=h2&&t!=t2){
				...
				for (int j = 0; j < h&&k<m; j++) {
					for (int i = 0; i < w; i++) {
						...
						boolean cond1=pixelcolor1.getRed()==0&&pixelcolor1.getGreen()==0&&pixelcolor1.getBlue()==0;
						boolean cond2=pixelcolor2.getRed()==0&&pixelcolor2.getGreen()==0&&pixelcolor2.getBlue()==0;
						boolean cond3=pixelcolor1.getRed()==255&&pixelcolor1.getGreen()==255&&pixelcolor1.getBlue()==255;
						boolean cond4=pixelcolor2.getRed()==255&&pixelcolor2.getGreen()==255&&pixelcolor2.getBlue()==255;
						if(cond1&&cond2){
							compt++;
							if(r%2==0){ r+=1;}
							if(b%2==0){ b+=1;}
